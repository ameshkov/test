...
Промежуточный экран куда пользователь попадает, когда жмет на кнопку типа "Learn about premium features". Или когда пытается включить какую-нибудь функцию, доступную только при покупке лицензии.

Здесь мы должны объяснить юзеру, что он получит при покупке.

## Manage License

Если приложение активировано лицензионным ключом, тут мы должны показать его статус, сколько осталось до конца, дать кнопку "Продлить" и "Сбросить лицензию".

## Send Feedback

Форма для связи с поддержкой. Наполнение можно оставить как есть + добавить опцию аттача скриншота.

## About Adguard

Информация о программе. Версия, полезные ссылки.
